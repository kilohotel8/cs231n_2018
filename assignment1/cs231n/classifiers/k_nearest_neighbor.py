...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    Input / Output: Same as compute_distances_two_loops
    """
    (np.shape(self.X_train))
    num_test = X.shape[0]
    num_train = self.X_train.shape[0]
    dists = np.zeros((num_test, num_train))
    X_square = np.sum(np.square(X),axis=1).reshape(np.shape(X)[0],1)  
    XX_train = np.dot(X,self.X_train.T)
    X_train_square = np.sum(np.square(self.X_train),axis=1)
    dists = np.sqrt(X_square - 2*XX_train + X_train_square)
    #########################################################################
    # TODO:                                                                 #
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      # the ith test point.
      closest_y = []
      closest_sort = np.argsort(dists[i,:])[:k]
      closest_y = self.y_train[closest_sort].astype(np.int32)      
      #########################################################################
      # TODO:                                                                 #
